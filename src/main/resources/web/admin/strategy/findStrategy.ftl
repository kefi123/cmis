...
<body>
	...
	<div class="container">
		<form class="form"
			action="getStrategyByFeature?page=1&size=4"
			...
			<div class="form-group col-md-12">
				<input type="text" class="form-control" placeholder="输入特征"
					name="st_feature" style="width: 400px; height: 40px; float: left">
		...
		<table class="table text-info" style="margin-top: 50px;">
			...
			<tbody id="Strategys">
				<#list pageInfos.list as strategy>
					<tr>
						<td>${strategy.st_id}</td>
						<td>${strategy.st_feature}</td>
						<td>${strategy.st_content}</td>
						<td><a href='deleteStrategy?st_id=${strategy.st_id}'
								 class='btn btn-info'>删除</a>
							&nbsp;&nbsp;&nbsp;&nbsp;
							<a href='getStrategyByID?st_id=${strategy.st_id}' class='btn btn-info'>修改</a>
						</td>
					</tr>
				</#list>
			</tbody>
